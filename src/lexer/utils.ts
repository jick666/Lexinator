...
export const isDigit = (ch: string | null): boolean =>
  ch !== null && ch >= '0' && ch <= '9';

/* helper for cheap char collection */
const join = (buf: string[]): string => (buf.length === 1 ? buf[0] : buf.join(''));
...
export function readUnicodeEscape(stream: CharStream): string | null {
...
export function consumeIdentifierLike(
  stream: CharStream,
  { unicode = true, allowEscape = false }: { unicode?: boolean; allowEscape?: boolean } = {}
): string | null {
...
/* ── numeric helpers ─────────────────────────────────────── */
export function readDigitsWithUnderscores(
  stream: CharStream,
  mark: Position
): { value: string; underscoreSeen: boolean; lastUnderscore: boolean } | null {
  const buf: string[] = [];
  let underscoreSeen = false,
    lastUnderscore = false,
    ch: string | null = stream.current();
...
export function readDigits(stream: CharStream): string {
  const buf: string[] = [];
  while (isDigit(stream.current())) { buf.push(stream.current()); stream.advance(); }
  return join(buf);
}

export function readNumberLiteral(
  stream: CharStream,
  mark: Position,
  requireFraction = false
): { value: string; ch: string | null } | null {
  const intPart = readDigits(stream);
  let buf: string[] = [intPart];
  let ch: string | null = stream.current();
...
export function consumeKeyword(
  stream: CharStream,
  kw: string,
  { checkPrev = true }: { checkPrev?: boolean } = {}
): Position | null {
  const mark = stream.getPosition();
  if (checkPrev) {
